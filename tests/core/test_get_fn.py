...
def test_get_rangefilter_fn() -> None:
    ...
    range_filter_fn = get_rangefilter_fn(minimum=0, maximum=100, unit="B", mapping={"B": 1}, is_enabled=False)
    assert range_filter_fn is None
    range_filter_fn = get_rangefilter_fn(minimum=0, maximum=100, unit="B", mapping={"B": 1}, is_enabled=True)
    assert isinstance(range_filter_fn, Callable)
    range_filter_fn = get_rangefilter_fn(minimum=-1, maximum=100, unit="B", mapping={"B": 1}, is_enabled=True)
    assert isinstance(range_filter_fn, Callable)
    range_filter_fn = get_rangefilter_fn(minimum=0, maximum=float("inf"), unit="B", mapping={"B": 1}, is_enabled=True)
    assert isinstance(range_filter_fn, Callable)
    range_filter_fn = get_rangefilter_fn(minimum=-1, maximum=float("inf"), unit="B", mapping={"B": 1}, is_enabled=True)
    assert range_filter_fn is None
