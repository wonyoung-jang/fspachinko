...
from ..utils import IconFilename, get_icon
...
class FileActions:
    ...
    def __post_init__(self) -> None:
        ...
        self.save = QAction(icon=QIcon(get_icon(IconFilename.SAVE)), text="&Save Profile")
        self.save.setShortcut("Ctrl+S")
        set_qt_tips(self.save, "Save current GUI profile (Ctrl+S)")

        self.save_as = QAction(icon=QIcon(get_icon(IconFilename.SAVE_AS)), text="Save Profile &As")
        self.save_as.setShortcut("Ctrl+Shift+S")
        set_qt_tips(self.save_as, "Save current GUI profile as ... (Ctrl+Shift+S)")

        self.load = QAction(icon=QIcon(get_icon(IconFilename.OPEN)), text="&Load Profile")
        self.load.setShortcut("Ctrl+O")
        set_qt_tips(self.load, "Load GUI profile (Ctrl+O)")

        self.autosave = QAction(
            icon=QIcon(get_icon(IconFilename.AUTOSAVE)),
        ...
        self.exit = QAction(icon=QIcon(get_icon(IconFilename.CLOSE)), text="&Exit")
...
class RunActions:
    ...
    def __post_init__(self) -> None:
        """Initialize the main actions."""
        self.start = QAction(icon=QIcon(get_icon(IconFilename.START)), text="&Start")
        self.start.setShortcut("Ctrl+R")
        set_qt_tips(self.start, "Start (Ctrl+R)")

        self.stop = QAction(icon=QIcon(get_icon(IconFilename.STOP)), text="S&top")
