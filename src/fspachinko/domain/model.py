...
class Engine:
    ...
    def process(self) -> None:
        ...
        self.events.append(ProcessStarted(self.dir_count))

        self.logging.info("Starting process with root: %s", self.root)

        for dir_i in range(1, self.dir_count + 1):
            target_qty = self.filecount_fn()

            self.events.append(DirectoryStarted(dir_i, target_qty))
    ...
    def process_dir(self, dst: DestinationDirectory) -> None:
        """Process one directory."""
        self.quota.reset()
        self.logging.add_handler(dst.path)

        entries = self.walker_fn()
        while True:
            if dst.is_success or self.is_stop_requested or self.root in self.quota.locked_dir:
                break
            if (e := next(entries, None)) is None:
                break
            self.process_file(dst, e)
    ...
    def process_file(self, dst: DestinationDirectory, e: FSEntry) -> None:
        """Process one file."""
        ...
        new_name = self.filesystem.get_unique_path(dst.path, new_stem, ext)  # I/O
        self.transfer_fn(e.path, new_name)
        self.quota.update(e)
        dst.count += 1
        dst.size += e.size
        self.logging.info("%s: %s -> %s", dst.count, e.path, new_name)

        self.events.append(FileTransferred(dst.count))
