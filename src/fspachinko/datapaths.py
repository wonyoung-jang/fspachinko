...
def data_path() -> str:
    ...
    return join(dirname(DefaultPath.APP), DefaultPath.DATA_DIR)


@cache
def icons_path() -> str:
    """Get the full path to the icons directory."""
    return join(data_path(), DefaultPath.ICON_DIR)


@cache
def configs_path() -> str:
    """Get the full path to the configs directory."""
    return join(data_path(), DefaultPath.CONFIG_DIR)


@cache
def logs_path() -> str:
    """Get the full path to the logs directory."""
    return join(data_path(), DefaultPath.LOG_DIR)
